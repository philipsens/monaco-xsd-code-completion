...
const complexType = (type) =>
    select('/xs:schema/xs:complexType[@name=/xs:schema/xs:element[@name="'+type+'"]/@type]/xs:sequence/xs:element/@name', parsedXsd)
const complexTypeChoice = (type) =>
    select('/xs:schema/xs:complexType[@name="'+type+'Type"]/xs:sequence/xs:choice/xs:element/@name', parsedXsd)
...
input.oninput = () => {
    ...
    output.innerHTML = '';
    let outputString;
    element().filter(node => node.nodeValue.includes(input.value))
...
input2.oninput = () => {
    ...
    output.innerHTML = '';
    let outputString;
    complexType(input.value).filter(node => node.nodeValue.includes(input2.value))
        .map(result => outputString += (result.nodeValue + "<br />"));
    output.innerHTML = outputString;
}
...
input3.oninput = () => {
    ...
    output.innerHTML = '';
    let outputString;
    complexTypeChoice(input2.value).filter(node => node.nodeValue.includes(input3.value))
        .map(result => outputString += (result.nodeValue + "<br />"));
    output.innerHTML = outputString;
}
